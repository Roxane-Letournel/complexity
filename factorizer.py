...
class Factorizer:
    ...
    def __init__(self, chunks):
        self.chunks = sorted(chunks, key=lambda t: len(
            t['word']), reverse=True)  # largest strings first
        self.bestCost = -1
        self.bestFact = []
        self.toDo = []

    def changeChunks(self, newChunks):
        self.chunks = sorted(newChunks, key=lambda t: len(
            t['word']), reverse=True)  # largest strings first
        self.bestCost = -1
        self.bestFact = []
        self.toDo = []
    ...
    def fastFactorize(self, stimulus):
        ...
        while(len(residual) != 0):
            ...
            for chunk in self.chunks:
                if chunk['word'] == residual[:min(
                        len(residual), len(chunk['word']))]:
            ...
            if residual and previousResidual == residual:
                raise NameError(
                    "Can't factorize: C is incomplete (missing canonical chunks)")
    ...
    def factorize(self, stimulus):
        ...
        while self.toDo:
            ...
            for chunk in self.chunks:
                if residual and chunk['word'] == residual[:min(
                        len(residual), len(chunk['word']))]:
                    ...
                    if currentCost_temp < self.bestCost:
                        ...
                        if not residual_temp:
                            ...
                            self.toDo += [(residual_temp,
                                           list(usedChunks_temp),
                                           currentCost_temp)]
