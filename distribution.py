...
# crée la liste des mots différents dans S_C et donne le nombre de mots
# différents et la longueur du mot le plus long
...
def aleaDistrib_Score(S_C, C):
    ...
    Sk = [sum([len(diffS_C[i]) == k for i in range(len(diffS_C))])
          for k in range(1, maxLen + 1)]  # contien le nombre de mots de longueur k
    return (1 / nbDiffS_C) * sum([Sk[k - 1] * proba(k, n)
